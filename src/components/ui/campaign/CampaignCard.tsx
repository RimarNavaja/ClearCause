...
const CampaignCard: React.FC<CampaignCardProps> = ({
  ...
  const progressPercentage = Math.min(
    (realtimeData.currentAmount / realtimeData.goalAmount) * 100,
    100
  );
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 border overflow-hidden h-full flex flex-col ${
        isAnimating
          ? "ring-2 ring-green-400 shadow-lg transform scale-[1.02]"
          : ""
      }`}
    >
